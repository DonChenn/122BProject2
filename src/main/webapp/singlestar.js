    ...
    fetch(`api/star?id=${starId}`)
        ...
        .then(text => {
            ...
            try {
                ...
                document.getElementById("star-details").innerHTML = `<p>Error parsing server response. Check console for details.</p><pre>${text}</pre>`;
                return;
            }
        ...
        .catch(error => {
            ...
            document.getElementById("star-details").innerHTML = `<p>Error loading star details: ${error.message}</p>`;
        });
}