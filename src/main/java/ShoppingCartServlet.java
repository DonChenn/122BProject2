    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (movieId == null || action == null || !cart.containsKey(movieId)) {
            ...
            if (updateSuccessful) {
                ...
                response.setStatus(HttpServletResponse.SC_OK);
                System.out.println("Cart updated: " + action + " for movie " + movieId);
            } else if (!response.isCommitted()){
                responseJsonObject.addProperty("status", "fail");
                responseJsonObject.addProperty("message", "Failed to update cart due to invalid action or other issue.");
